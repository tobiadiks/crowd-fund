...
fn process_instruction(
    ...
    msg!("Did't find the entrypoint required");
...
fn create_campaign(
    ...
    let mut input_data = CampaignDetails::try_from_slice(&instruction_data)
        .expect("instruction data serialization did't work");
    ...
    let rent_exempt = Rent::get()?.minimum_balance(writing_account.data_len());

    if **writing_account.lamports.borrow() < rent_exempt {
        msg!("The balance of writing account should be more than rent_exempt");
